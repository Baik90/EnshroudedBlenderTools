...
local custom_loca = game.assets.create_resource({{
    keenglish = "{mod_id}",
    description = "Custom item generated by Enshrouded Blender Tools"
}}, "keen::LocaTag")
if custom_loca == nil then
    error("[{mod_id}] could not create custom LocaTag")
end
custom_item.data.objectId = custom_item.guid
custom_item.data.itemId.value = game.guid.hash(custom_item.guid)
custom_item.data.name = custom_loca
custom_item.data.caption = custom_loca
custom_item.data.description = custom_loca
...
recipe_info.recipeName = custom_loca
